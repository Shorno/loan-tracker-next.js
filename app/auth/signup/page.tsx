...
export default function SignupForm() {
    const { register, formState: { errors, isSubmitting }, setError } = useForm<SignupFormData>({
        mode: "all",
        resolver: zodResolver(signupSchema),
        defaultValues: {
            name: "",
    ...
    useEffect(() => {
        if (state.error) {
            if (state.error.includes("duplicate key value violates unique constraint")) {
                setError("email", {
                    type: "manual",
                    message: "This email is already in use."
                });
            } else if (state.error.includes("Invalid data provided")) {
